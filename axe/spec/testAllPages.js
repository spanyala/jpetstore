...
describe('Radio button demo', function() {
    ...
    beforeEach(function(done) {
        ...
        driver.get('http://jp-dev.salientcrgt-devops.com/jpetstore/actions/Catalog.action?viewCategory=&categoryId=FISH')
            .then(function () {
                done();
            });
    });
    ...
    it('should fetch the Fish page and analyze it', function (done) {
		driver.findElement(selenium.By.id("Catalog"))
            .then(function () {
                AxeBuilder(driver)
                    .analyze(function(results) {
                        ...
                        if (results.violations.length > 0) {
                            //console.log(util.inspect(results.violations, true, null));
			    var stream = fs.createWriteStream("testFish.html");
       			    stream.once('open', function(fd) {
        			for (i=0; i<results.violations.length; i++) {
         				stream.write("Help: " + results.violations[i].help + "<br/>\n");
         				stream.write("Description:  " + results.violations[i].description + "<br/>\n");
            ...
            .then(function () {
                AxeBuilder(driver)
                    .analyze(function(results) {
                        ...
                        if (results.violations.length > 0) {
                            //console.log(util.inspect(results.violations, true, null));
			    var stream = fs.createWriteStream("testKoi.html");
       			    stream.once('open', function(fd) {
        			for (i=0; i<results.violations.length; i++) {
         				stream.write("Help: " + results.violations[i].help + "<br/>\n");
         				stream.write("Description:  " + results.violations[i].description + "<br/>\n");
